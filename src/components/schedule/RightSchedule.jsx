...
const RightSchedule = (props) => {
  ...
  useEffect(() => {
    const checkElementPosition = () => {
      if (elementRef.current) {
        ...
        const threshold = 225; // You can adjust this value as needed
  ...
  return (
    <div
      ...
      />
      <div
        className={`absolute left-0 h-full border-r-0 rounded-l-[50px] w-1/2 border-[#d946ef] border-[0.7rem] border-solid transition-all ease-in-out duration-1000 ${posStyles} ${
          isAbove || isCentered ? "border-[#d946ef]" : "border-white"
        }`}></div>
      <div
        className={`${
          isCentered
            ? "h-12 border-[15px] border-white left-[-1.1rem] bg-[#d946ef]"
            : "h-8 bg-white left-[-0.6rem] border-[#d946ef] border-[10px]"
