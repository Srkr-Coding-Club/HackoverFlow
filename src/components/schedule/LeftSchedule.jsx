...
const LeftSchedule = (props) => {
  ...
  return (
    <div className="relative flex w-full" style={{ top: `-${top}rem` }}>
      ...
      />
      <div
        className={`absolute right-0 h-full border-l-0 rounded-r-[50px] w-1/2 border-blue-600 border-[0.70rem] border-solid transition-all ease-in-out duration-1000 ${posStyles} ${
          isAbove || isCentered ? " border-blue-600" : "border-white"
        }`}></div>
      <div
        className={`${
          isCentered
            ? "h-12 border-[15px] border-white right-[-1.1rem] bg-blue-600"
            : "h-8 bg-white right-[-0.6rem] border-blue-600 border-[10px]"
